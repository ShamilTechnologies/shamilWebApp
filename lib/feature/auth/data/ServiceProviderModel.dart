...
class SubscriptionPlan extends Equatable {
  ...
  factory SubscriptionPlan.fromMap(Map<String, dynamic> map) {
    return SubscriptionPlan(
      ...
      price: (map['price'] as num?)?.toDouble() ?? 0.0, // Default to 0.0 if null
      description: map['description'] as String? ?? '', // Default to empty string if null
      duration: map['duration'] as String? ?? '', // Default to empty string if null
    );
  }
...
class OpeningHours extends Equatable {
  final Map<String, Map<String, String>> hours; // e.g., {'monday': {'open': '09:00', 'close': '18:00'}, 'tuesday': ...}
...
class ServiceProviderModel extends Equatable {
  ...
  final String businessContactEmail; // <-- ADDED: Business specific contact email
  ...
  final List<String> amenities; // List of amenity strings (e.g., "WiFi", "Parking")
  ...
  final List<SubscriptionPlan>? subscriptionPlans; // List of plans if model is subscription
  final double? reservationPrice; // Price per reservation if model is reservation
  ...
  final bool isRegistrationComplete; // Flag set when all registration steps are done
  ...
  final Timestamp? updatedAt; // When the document was last updated (server timestamp)
  final double rating; // <-- ADDED: Average rating (likely calculated)
  final int ratingCount; // <-- ADDED: Total number of ratings (likely calculated)
  final List<String>? staffUids; // <-- ADDED: Optional list of staff UIDs with access
  ...
  factory ServiceProviderModel.empty(String uid, String email) {
    ...
    return ServiceProviderModel(
      ...
      address: const {'street': '', 'city': '', 'governorate': '', 'postalCode': ''},
      location: null,
  ...
  factory ServiceProviderModel.fromFirestore(DocumentSnapshot doc) {
    ...
    if (data['openingHours'] != null && data['openingHours'] is Map) {
       try { hours = OpeningHours.fromMap(Map<String, dynamic>.from(data['openingHours'])); }
       catch (e) { print("Error deserializing openingHours for ${doc.id}: $e"); hours = null; }
    }

    List<SubscriptionPlan>? plans;
    if (data['subscriptionPlans'] != null && data['subscriptionPlans'] is List) {
      try {
        plans = (data['subscriptionPlans'] as List)
            .where((item) => item is Map) // Ensure items are maps
            .map((planData) => SubscriptionPlan.fromMap(Map<String, dynamic>.from(planData)))
            .toList();
      } catch (e) { print("Error deserializing subscriptionPlans for ${doc.id}: $e"); plans = []; }
    }
    ...
    if (data['pricingModel'] is String) {
      try {
         pricing = PricingModel.values.firstWhere(
           ...
           orElse: () { print("Warning: Unknown pricingModel value '${data['pricingModel']}' for ${doc.id}. Defaulting to 'other'."); return PricingModel.other; }
         );
      } catch (e) { print("Error deserializing pricingModel for ${doc.id}: $e"); pricing = PricingModel.other; }
    }
    ...
    return ServiceProviderModel(
      ...
      ownerUid: data['ownerUid'] as String? ?? (data['uid'] as String? ?? doc.id), // Fallback ownerUid to uid/doc.id if missing
      // Personal
      ...
      businessContactEmail: data['businessContactEmail'] as String? ?? '', // <-- ADDED
      ...
      // Operations
      openingHours: hours,
      amenities: data['amenities'] != null ? List<String>.from(data['amenities']) : const [], // <-- ADDED
      servicesOffered: data['servicesOffered'] != null ? List<Map<String, dynamic>>.from(data['servicesOffered']) : const [], // <-- ADDED
      // Pricing
      ...
      galleryImageUrls: data['galleryImageUrls'] != null ? List<String>.from(data['galleryImageUrls']) : const [], // <-- RENAMED
      // Metadata & Status
      ...
      createdAt: data['createdAt'] is Timestamp ? data['createdAt'] : Timestamp.now(), // Fallback for safety
      ...
      staffUids: data['staffUids'] != null ? List<String>.from(data['staffUids']) : const [], // <-- ADDED
    );
  }
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      if (personalPhoneNumber.isNotEmpty) 'personalPhoneNumber': personalPhoneNumber,
      ...
      if (businessContactPhone.isNotEmpty) 'businessContactPhone': businessContactPhone,
      if (businessContactEmail.isNotEmpty) 'businessContactEmail': businessContactEmail, // <-- ADDED
      ...
      if (servicesOffered.isNotEmpty) 'servicesOffered': servicesOffered, // <-- ADDED
      // Pricing
      'pricingModel': pricingModel.name, // Store enum name as string
      if (subscriptionPlans != null && subscriptionPlans!.isNotEmpty)
        'subscriptionPlans': subscriptionPlans!.map((plan) => plan.toMap()).toList(),
      ...
      if (galleryImageUrls != null && galleryImageUrls!.isNotEmpty) 'galleryImageUrls': galleryImageUrls, // <-- RENAMED
      // Metadata & Status
      ...
      if (staffUids != null && staffUids!.isNotEmpty) 'staffUids': staffUids, // <-- ADDED
    };
  }
  ...
  ServiceProviderModel copyWith({
    // Core
    String? uid, String? ownerUid,
    // Personal
    String? name, String? email, DateTime? dob, String? gender, String? personalPhoneNumber,
    String? idNumber, String? idFrontImageUrl, String? idBackImageUrl,
    // Business
    String? businessName, String? businessDescription, String? businessContactPhone, String? businessContactEmail, String? website, String? businessCategory,
    // Location
    Map<String, String>? address, GeoPoint? location,
    // Operations
    OpeningHours? openingHours, List<String>? amenities, List<Map<String, dynamic>>? servicesOffered,
    // Pricing
    PricingModel? pricingModel, List<SubscriptionPlan>? subscriptionPlans, double? reservationPrice, String? pricingInfo,
    // Assets
    ...
    return ServiceProviderModel(
      ...
      // Personal
      name: name ?? this.name, email: email ?? this.email, dob: dob ?? this.dob, gender: gender ?? this.gender,
      personalPhoneNumber: personalPhoneNumber ?? this.personalPhoneNumber, idNumber: idNumber ?? this.idNumber,
      ...
      businessContactPhone: businessContactPhone ?? this.businessContactPhone, businessContactEmail: businessContactEmail ?? this.businessContactEmail,
      website: website ?? this.website, businessCategory: businessCategory ?? this.businessCategory,
      // Location
      address: address ?? this.address, location: location ?? this.location,
      // Operations
      openingHours: openingHours ?? this.openingHours, amenities: amenities ?? this.amenities, servicesOffered: servicesOffered ?? this.servicesOffered,
      // Pricing
      pricingModel: pricingModel ?? this.pricingModel, subscriptionPlans: subscriptionPlans ?? this.subscriptionPlans,
      reservationPrice: reservationPrice ?? this.reservationPrice, pricingInfo: pricingInfo ?? this.pricingInfo,
      // Assets
      logoUrl: logoUrl ?? this.logoUrl, mainImageUrl: mainImageUrl ?? this.mainImageUrl, galleryImageUrls: galleryImageUrls ?? this.galleryImageUrls,
      // Metadata & Status
      ...
      rating: rating ?? this.rating, ratingCount: ratingCount ?? this.ratingCount, staffUids: staffUids ?? this.staffUids,
    );
  }

  // --- Equatable Implementation ---
  @override
  List<Object?> get props => [ // Ensure all fields are included for correct comparison
        // Core
        uid, ownerUid,
        // Personal
        ...
        // Business
        businessName, businessDescription, businessContactPhone, businessContactEmail, website, businessCategory,
        // Location
        address, location,
        // Operations
  ...
  bool isPersonalDataValid() {
    ...
    final bool isEmailValid = email.isNotEmpty && emailValidate(email); // Should always be true post-auth
    final bool isPhoneValid = personalPhoneNumber.isNotEmpty; // Basic check
    final bool isValid = name.isNotEmpty && isEmailValid && (dob != null) &&
        (gender != null && gender!.isNotEmpty) && isPhoneValid &&
        idNumber.isNotEmpty && idFrontImageUrl != null && idFrontImageUrl!.isNotEmpty &&
        idBackImageUrl != null && idBackImageUrl!.isNotEmpty;
    print("Personal Data Validation Result (Step 1): $isValid (Name: '$name', DOB: $dob, Gender: '$gender', Phone: '$personalPhoneNumber', IDNum: '$idNumber', Imgs: ${idFrontImageUrl!=null && idBackImageUrl!=null})");
    return isValid;
  }
  ...
  bool isBusinessDataValid() {
    ...
    final bool addressValid = address['street'] != null && address['street']!.isNotEmpty &&
                              address['city'] != null && address['city']!.isNotEmpty &&
                              address['governorate'] != null && address['governorate']!.isNotEmpty;
    final bool isValid = businessName.isNotEmpty && businessDescription.isNotEmpty &&
        businessContactPhone.isNotEmpty && businessCategory.isNotEmpty &&
        addressValid && location != null && // Location is required
     ...
     print("Business Data Validation Result (Step 2): $isValid (Name: '$businessName', Cat: '$businessCategory', AddrValid: $addressValid, Loc: ${location!=null}, Hours: ${openingHours!=null})");
     return isValid;
  }
  ...
  bool isPricingValid() {
    ...
    bool isValid;
    switch (pricingModel) {
      case PricingModel.subscription: isValid = subscriptionPlans != null && subscriptionPlans!.isNotEmpty; break;
      case PricingModel.reservation: isValid = reservationPrice != null && reservationPrice! >= 0; break;
      case PricingModel.other: isValid = true; break; // 'Other' might not require specific data fields
    }
     ...
     print("Pricing Data Validation Result (Step 3): $isValid (Model: ${pricingModel.name})");
     return isValid;
  }
  ...
  bool isAssetsValid() {
    ...
    final isValid = logoUrl != null && logoUrl!.isNotEmpty &&
                  mainImageUrl != null && mainImageUrl!.isNotEmpty;
    // Gallery images are likely optional
    print("Assets Data Validation Result (Step 4): $isValid (Logo: ${logoUrl!=null}, MainImg: ${mainImageUrl!=null})");
    return isValid;
  }
  ...
  int get currentProgressStep {
    ...
    if (!isPersonalDataValid()) { print("Resume Step: 1 (Personal data invalid)"); return 1; }
    if (!isBusinessDataValid()) { print("Resume Step: 2 (Business data invalid)"); return 2; }
    if (!isPricingValid()) { print("Resume Step: 3 (Pricing data invalid)"); return 3; }
    if (!isAssetsValid()) { print("Resume Step: 4 (Assets data invalid)"); return 4; }
