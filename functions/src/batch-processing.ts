 ...
 */
export const syncAccessLogs = functions.https.onCall(async (data, context) => {
  ...
  try {
    const logs = data.logs as AccessLog[];
 ...
 */
export const processPendingLogs = functions.pubsub.schedule('every 24 hours').onRun(async (context) => {
  try {
    ...
    const logsByProvider: {[key: string]: any[]} = {};
    ...
    snapshot.forEach(doc => {
      ...
      logsByProvider[providerId].push({
        id: doc.id,
        ...data
      });
    });
